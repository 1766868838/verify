...
#    - 功能：执行两个指定数据库的深度对比
#    - 参数：
#      server1_val/server2_val：服务器连接信息字典(包含host,user,password等)
#      db1/db2：要对比的数据库名
#      options：配置选项字典，支持以下关键参数：
#        - quiet：静默模式开关
#        - run_all_tests：遇到错误是否继续执行
#        - no_diff：跳过对象定义对比
#        - no_row_count：跳行行数校验
#        - no_data：跳过数据一致性检查
...
#    - 功能：对比两个服务器上所有非系统数据库
#    - 参数：
#      exclude_list：要排除的数据库列表
#      options：包含'changes-for'指定对比方向等配置
